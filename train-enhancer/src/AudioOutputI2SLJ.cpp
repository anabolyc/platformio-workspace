...
AudioOutputI2SLJ::AudioOutputI2SLJ(int port, int output_mode, int dma_buf_count, int use_apll)
{
  this->portNo = port;
  this->i2sOn = false;
...
AudioOutputI2SLJ::~AudioOutputI2SLJ()
{
#ifdef ESP32
  if (i2sOn) {
...
bool AudioOutputI2SLJ::SetPinout(int bclk, int wclk, int dout)
{
#ifdef ESP32
...
bool AudioOutputI2SLJ::SetRate(int hz)
{
...
bool AudioOutputI2SLJ::SetBitsPerSample(int bits)
{
...
bool AudioOutputI2SLJ::SetChannels(int channels)
{
...
bool AudioOutputI2SLJ::SetOutputModeMono(bool mono)
{
  this->mono = mono;
  return true;
}

bool AudioOutputI2SLJ::begin()
{
  return true;
}
...
bool AudioOutputI2SLJ::ConsumeSample(int16_t sample[2])
{
  int16_t ms[2];
...
#else
  uint32_t this_sample = ((ms[RIGHTCHANNEL])<<16) | (ms[LEFTCHANNEL] & 0xffff);
  uint32_t s32 = (prev_sample << 1) | ((this_sample >> 31) & 0x0001);
